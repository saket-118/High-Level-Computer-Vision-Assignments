...
class BaseTrainer:
    ...
    def __init__(self, config, log_dir):
        ...
        # General config (model arch, optimizer, lr_scheduler, etc.)
        self.config = config
        # Training details such as num_epochs etc.
        ...
        if self.trainer_config['tensorboard']:
            self.writer = TensorboardWriter(
                ospj(log_dir, 'tensorboard'), self.logger)
        self.wandb_enabled = self.trainer_config['wandb']

        # Prepare for (multi-device) GPU training
        # This part doesn't do anything if you don't have a GPU.
        self._device, self._device_ids = prepare_device(
            self.trainer_config['n_gpu'])
    ...
    def _configure_logging(self, log_dir):
        ...
        if os.path.exists(_log_file):
            print(
                f'Warning! Log file {_log_file} already exists! The logs will be appended!')
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            if self.should_evaluate():
                ...
                log.update({f'eval_{key}': value for key,
                           value in eval_result.items()})
            ...
            if self.monitor_mode != 'off':  # Then there is a metric to monitor
                if self.monitor_metric in log:  # Then we have measured it in this epoch
                    #################################################################################################
                    # Q2.b: Check if this is the best epoch, based on self.monitor_metric. If so, save best model.  #
                    #################################################################################################
                    metric_val = log[self.monitor_metric]
                    improved = (self.monitor_mode == 'min' and metric_val < self.monitor_best) or \
                               (self.monitor_mode == 'max' and metric_val > self.monitor_best)

                    if improved:
                        self.monitor_best = metric_val
                        self.best_epoch = self.current_epoch
                        self.not_improved_count = 0
                        best_path = os.path.join(
                            self.checkpoint_dir, 'best_val_model.pth')
                        # Commented out best‐model save for Q4(a)
                        # self.save_model(path=best_path)
                        self.logger.info(f"*** (BEST) epoch {self.current_epoch}, "
                                         f"{self.monitor_metric}={metric_val:0.5f}")
                    else:
                        self.not_improved_count += 1

                    # Q2.c: early‐stopping check
                    if self.not_improved_count >= self.early_stop:
                        self.logger.info(f"Early stopping: no improvement for "
                                         f"{self.early_stop} consecutive evals.")
                        break

                else:
                    # The metric wasn't measured in this epoch. Don't change not_improved_count.
            ...
            # ──────────────────────────────────────────────────────────────
            # Periodic checkpoint block: only execute if save_period > 0
            if self.save_period > 0:
                if self.current_epoch % self.save_period == 0:
                    # Commented out to disable checkpointing entirely:
        ...
        # ──────────────────────────────────────────────────────────────
        # FINAL “save last model” step: comment out entirely for Q4(a):
