...
class TwoLayerNetv1(object):
    ...
    def forward(self, X):
        ...
        self.cache = {'X': X,'z1': z2,'h1': a2,'softmax_probs': softmax_scores,}
...
class TwoLayerNetv2(TwoLayerNetv1):
    ...
    def compute_loss(self, X, y=None, reg=0.0):
        ...
        #############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        log_probs = -np.log(softmax_scores[np.arange(N), y])  # -log(prob) for correct class
        data_loss = np.sum(log_probs) / N  # Average over N samples
...
class TwoLayerNetv3(TwoLayerNetv2):
    ...
    def back_propagation(self, X, y=None, reg=0.0):
        ...
        #############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        loss = super().compute_loss(X, y, reg)
        ...
        grads['W1'] = Xc.T.dot(dz1) + 2 * reg * W1        
...
class MultiLayerPerceptron(nn.Module):
    def __init__(self, input_size, hidden_layers, num_classes, drop_prob=0.0):
    ...
    def forward(self, x):
        return self.layers(x.view(x.size(0), -1))  # Flatten input
